...
public class Utility {
    ...
    public static boolean adbStart(Context context) {
        try {
            if (!MainActivity.USB_DEBUG) {
                Utility.setProp("service.adb.tcp.port", MainActivity.PORT);
                try {
                    if (Utility.isProcessRunning("adbd")) {
                        Utility.runRootCommand("stop adbd");
                    }
                } catch (Exception e) {
                }
                Utility.runRootCommand("start adbd");
            }
            try {
            ...
            // Try to auto connect
            if (Utility.prefsAutoCon(context)) {
                Utility.autoConnect(context, "c");
            }

            // Wake Lock
            if (Utility.prefsScreenOn(context)) {
                final PowerManager pm = (PowerManager) context.getSystemService(Context.POWER_SERVICE);
                mWakeLock = pm.newWakeLock(PowerManager.SCREEN_BRIGHT_WAKE_LOCK, context.getClass().getName());
                mWakeLock.acquire();
            }

            if (Utility.prefsNoti(context)) {
                manager = (NotificationManager) context.getSystemService(context.NOTIFICATION_SERVICE);
                Utility.showNotification(context, R.drawable.bug_on, "ADB is active at" + " " + Utility.getWifiIp(context)+":"+prefsAutoConPort(context));
            }
        } catch (Exception e) {
            return false;
        }
    ...
    public static void autoConnect(Context context, String mode) {
        String autoConIP = Utility.prefsAutoConIP(context);
        String autoConPort = Utility.prefsAutoConPort(context);
        if (autoConIP.trim().equals("") || autoConPort.trim().equals("")) {
            Toast.makeText(context, "Incomplete", Toast.LENGTH_LONG).show();
            return;
        }
        String urlRequest = "http://" + autoConIP + ":" + autoConPort + "/" + mode + "/" + Utility.getWifiIp(context);
        try {
    ...
    public static void enableWiFi(Context context, boolean enable) {
        if (enable) {
            Toast.makeText(context, "Turning Wifi Off", Toast.LENGTH_LONG).show();
        } else {
    ...
    public static void showNotification(Context context, int icon, String text) {
        ...
        PendingIntent pendingIntent = PendingIntent.getActivity(context,START_NOTIFICATION_ID,intent,0);

        Notification.Builder builder = new Notification.Builder(context);

        builder.setAutoCancel(false);
        builder.setContentTitle("ADB");
        builder.setContentText(text);
        builder.setSmallIcon(R.drawable.bug_on);
        builder.setOngoing(true);
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            builder.addAction(new Notification.Action(R.drawable.bug_on,"OFF", pendingIntent));
        }
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            builder.setColor(Color.BLACK);
        }
        builder.setSubText("adb is turned on");   //API level 16
        builder.build();
        Notification notification = builder.getNotification();
        Utility.manager.notify(Utility.START_NOTIFICATION_ID, notification);
    }
    ...
    public static String prefsAutoConPort(Context context) {
        SharedPreferences pref = PreferenceManager.getDefaultSharedPreferences(context);
        return pref.getString(context.getResources().getString(R.string.pref_autoconport_key), "5555");
    }
